...
# Read the unified patient metadata file into a DataFrame
df2 = pd.read_csv(pcr_profile, sep='\t')

# Merge the two DataFrames on the 'Sample' column
merged_df = pd.merge(df1, df2[['Sample', 'PCR']], on='Sample', how='left')

# Move the PCR column to the second position
cols = list(merged_df.columns)
cols.insert(1, cols.pop(cols.index('PCR')))
