...
# Step 1: Read the first file and collect prefixes, samples, PCR status, and Total_Reads
prefix_info = []
with open(all_Kraken_Kaiju_combined_blast_results_pcr, 'r') as f:
    ...
    for row in reader:
        if row['Blastn_Support'] == 'Y':
            ...
            prefix_info.append((prefix, sample, pcr, total_reads))
...
with open(all_blast_results_virus_lineages, 'r') as f:
    ...
    for row in reader:
        ...
        for prefix, sample, pcr, total_reads in prefix_info:
            if query_id.startswith(prefix):
                ...
                try:
                    ...
                    continue
                filtered_rows.append((sample, pcr, total_reads, virus_class, family, genus, species, perc_identity, align_length))
                break  # Once a prefix matches, move to the next row

# Step 3: Group by sample, PCR, Total_Reads, and taxonomic classification and compute averages
group_stats = defaultdict(lambda: {'count': 0, 'sum_identity': 0.0, 'sum_align': 0})
for entry in filtered_rows:
    sample, pcr, total_reads, v_class, v_family, v_genus, v_species, perc_id, align_len = entry
    key = (sample, pcr, total_reads, v_class, v_family, v_genus, v_species)
...
for key in group_stats:
    sample, pcr, total_reads, v_class, v_family, v_genus, v_species = key
...
with open(virus_summary, 'w', newline='') as f:
    ...
    writer.writerow([
        'Sample', 'PCR', 'Virus_Class', 'Virus_Family', 'Virus_Genus', 'Virus_Species','Total_Reads', 'Number_of_supporting_reads', 'RPM' ,
